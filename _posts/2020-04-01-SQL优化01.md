...
本示例通过***MySQL***官方案例库***sakila***，下载地址[https://dev.mysql.com/doc/index-other.html](https://dev.mysql.com/doc/index-other.html)。压缩包包括三个文件***sakila-schema.sql***、***sakila-data.sql***、***sakila.mwb***，分别是***sakila***库的结构创建、数据插入、sakila的***MySQL Workbench***数据模型（可以在***MySQL***工作台打开查看数据库模型）。
